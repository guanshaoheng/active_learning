...
def getFilesPathList(root_path_list, maxTime, explicit_flag= False, add_flag=False, series_flag=False):
    ...
    for pathTemp in root_path_list:
        ...
        for i in range(0, maxTime + 1):
            temp1 = 'time_%d.dat' % i
            if temp1 in file_name_list and i == 0:
        ...
        if explicit_flag:
            ...
            for i in file_name_list:
                if len(i.split('_')) == 3:
                    temp.append(i)
            temp_index = np.argsort(np.array([int(i.split('_')[1]) for i in temp]))
            for i in temp_index:
                file_list.append(os.path.join(file_path, temp[i]))
...
def get_data_series(
        root_path_list,  maxTime=100, mixflag=False, numg=484, series_flag=False, explicit_flag=False):
    '''
    ...
    data_dic = get_data(
        ...
        explicit_flag=explicit_flag, series_flag=series_flag)
